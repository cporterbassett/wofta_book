...
Rows = KEEP list from cleanup_keep.sh  ∪  every abc/*-final.abc tune.
Auto-columns (regenerated every run): Health, key_status, Finalized?,
  Key, Meter, Chords, Title, Measures, Truncation.
Human columns (NOT overwritten): "Verified?" and "Notes" — parsed back out of
...
def build_tune_list():
    """KEEP list first (in order), then any extra tunes that have a -final.abc."""
    keep = load_keep_list()
...
def build():
    ...
    for tune in all_tunes:
        ...
        verified = (human.get('Verified?') or 'No').strip() or 'No'
    ...
    out.append('_Auto-generated by `build_tracking_sheet.py`. The auto columns are '
               'overwritten on every run; the **Verified?** and **Notes** columns are '
               'preserved (edit them freely)._')
